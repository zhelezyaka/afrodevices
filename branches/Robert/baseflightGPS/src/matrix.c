...
void set_identity_matrix_value(Matrix *m,  float value) {
	assert(m->rows == m->cols);
	int i, j;
	for (i = 0; i < m->rows; ++i) {
		for (j = 0; j < m->cols; ++j) {
...
int destructive_invert_matrix(Matrix *input, Matrix *output) {
	...
	/* Convert input to the identity matrix via elementary row operations.
	 The i'th pass through this loop turns the element at i,i to a 1
	...
	for (i = 0; i < input->rows; ++i) {
		...
		float scalar = 1.0 / input->data[i][i];
		...
		for (j = 0; j < input->rows; ++j) {
			...
			float shear_needed = -input->data[j][i];
