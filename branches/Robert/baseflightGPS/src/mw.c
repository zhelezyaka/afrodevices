...
void annexCode(void)
{
    ...
    // PITCH & ROLL only dynamic PID adjustemnt,  depending on throttle value <-- stragnieri stupido
    ...
    if ((calibratingA > 0 && getSensors(SENSOR_ACC)) || (calibratingG > 0)) {      // Calibration phasis
        LED0_TOGGLE;
    } else {
        if (flightMode.ACC_CALIBRATED)
            LED0_OFF;
        if (flightMode.ARMED)
            LED0_ON;
        // This will switch to/from 9600 or 115200 baud depending on state. Of course, it should only do it on changes.
        if (getFeature(FEATURE_TELEMETRY))
            initTelemetry(flightMode.ARMED);
    }

#ifdef LEDRING
    if (getFeature(FEATURE_LED_RING)) {
    ...
    if ((int32_t)(currentTime - calibratedAccTime) >= 0) {
        if (!flightMode.SMALL_ANGLES_25) {
            flightMode.ACC_CALIBRATED = 0; // the multi uses ACC and is not calibrated or is too much inclinated
            LED0_TOGGLE;
            calibratedAccTime = currentTime + 500000;
        } else {
            flightMode.ACC_CALIBRATED = 1;
        }
    }

    serialCom();

    if (getSensors(SENSOR_GPS)) {
...
void loop(void)
{
    ...
    if ((int32_t)(currentTime - rcTime) >= 0) { // 50Hz
        ...
        // Failsafe routine
        if (getFeature(FEATURE_FAILSAFE)) {
            if (failsafeCnt > (5 * cfg.failsafe_delay) && flightMode.ARMED) { // Stabilize, and set Throttle to specified level
                for (i = 0; i < 3; i++)
                ...
                if (failsafeCnt > 5 * (cfg.failsafe_delay + cfg.failsafe_off_delay)) {  // Turn OFF motors after specified Time (in 0.1sec)
                    flightMode.ARMED = 0;  // This will prevent the copter to automatically rearm if failsafe shuts it down and prevents
                    flightMode.OK_TO_ARM = 0;        // to restart accidentely by just reconnect to the tx - you will have to switch off first to rearm
                }
                failsafeEvents++;
            }
            if (failsafeCnt > (5 * cfg.failsafe_delay) && !flightMode.ARMED) {  // Turn off "Ok To arm to prevent the motors from spinning after repowering the RX with low throttle and aux to arm
                flightMode.ARMED = 0;        // This will prevent the copter to automatically rearm if failsafe shuts it down and prevents
                flightMode.OK_TO_ARM = 0;    // to restart accidentely by just reconnect to the tx - you will have to switch off first to rearm
            }
            failsafeCnt++;
        ...
        if (rcData[THROTTLE] < cfg.mincheck) {
            ...
            errorAngleI[PITCH] = 0;
            rcDelayCommand++;
            if (rcData[YAW] < cfg.mincheck && rcData[PITCH] < cfg.mincheck && !flightMode.ARMED) {
                if (rcDelayCommand == 20) {
                    calibratingG = 1000;
                    if (getFeature(FEATURE_GPS))
                        GPS_reset_home_position();
                }
            } else if (getFeature(FEATURE_INFLIGHT_ACC_CAL) && (!flightMode.ARMED && rcData[YAW] < cfg.mincheck && rcData[PITCH] > cfg.maxcheck && rcData[ROLL] > cfg.maxcheck)) {
                if (rcDelayCommand == 20) {
                ...
                if (rcOptions[BOXARM] && flightMode.OK_TO_ARM) {
                    ...
                    flightMode.ARMED = 0;
                rcDelayCommand = 0;
            } else if ((rcData[YAW] < cfg.mincheck || (cfg.retarded_arm == 1 && rcData[ROLL] < cfg.mincheck)) && flightMode.ARMED) {
                if (rcDelayCommand == 20)
                    flightMode.ARMED = 0;  // rcDelayCommand = 20 => 20x20ms = 0.4s = time to wait for a specific RC command to be acknowledged
            } else if ((rcData[YAW] > cfg.maxcheck || (rcData[ROLL] > cfg.maxcheck && cfg.retarded_arm == 1)) && rcData[PITCH] < cfg.maxcheck && !flightMode.ARMED && calibratingG == 0 && flightMode.ACC_CALIBRATED) {
                if (rcDelayCommand == 20) {
                    flightMode.ARMED = 1;
                    headFreeModeHold = heading;
                    resetIntegrator();
                ...
                rcDelayCommand = 0;
        } else if (rcData[THROTTLE] > cfg.maxcheck && !flightMode.ARMED) {
            if (rcData[YAW] < cfg.mincheck && rcData[PITCH] < cfg.mincheck) {   // throttle=max, yaw=left, pitch=min
                ...
                if (rcDelayCommand == 20)
                    flightMode.CALIBRATE_MAG = 1;   // MAG calibration request
                rcDelayCommand++;
        ...
        if (getFeature(FEATURE_INFLIGHT_ACC_CAL)) {
            if (AccInflightCalibrationArmed && flightMode.ARMED && rcData[THROTTLE] > cfg.mincheck && !rcOptions[BOXARM]) {   // Copter is airborne and you are turning it off via boxarm : start measurement
            ...
            if (rcOptions[BOXPASSTHRU]) {      // Use the Passthru Option to activate : Passthru = TRUE Meausrement started, Land and passtrhu = 0 measurement stored
                if (!AccInflightCalibrationActive && !AccInflightCalibrationMeasurementDone)
                    InflightcalibratingA = 50;
            } else if (AccInflightCalibrationMeasurementDone && !flightMode.ARMED) {
                AccInflightCalibrationMeasurementDone = 0;
                AccInflightCalibrationSavetoEEProm = 1;
            }
        }
        ...
        if ((rcOptions[BOXANGLE] || (failsafeCnt > 5 * cfg.failsafe_delay)) && (getSensors(SENSOR_ACC))) {
            // bumpless transfer to Level mode
            if (!flightMode.ANGLE_MODE) {
                errorAngleI[ROLL] = 0;
                errorAngleI[PITCH] = 0;
                flightMode.ANGLE_MODE = 1;
            }
        } else {
            flightMode.ANGLE_MODE = 0;        // failsave support
        }
        ...
        if (rcOptions[BOXHORIZON]) {
            if (!flightMode.HORIZON_MODE) {
                errorAngleI[ROLL] = 0; 
                errorAngleI[PITCH] = 0;
                flightMode.HORIZON_MODE = 1;
            }
        } else {
            flightMode.HORIZON_MODE = 0;
        }

        if ((rcOptions[BOXARM]) == 0)
            flightMode.OK_TO_ARM = 1;
...
#ifdef BARO
        if (getSensors(SENSOR_BARO)) {
            if (rcOptions[BOXBARO]) {
                if (!flightMode.BARO_MODE) {
                    flightMode.BARO_MODE = 1;
                    AltHold = EstAlt;
                    resetIntegrator();
                ...
                flightMode.BARO_MODE = 0;
        }
#endif

#ifdef  MAG
        if (getSensors(SENSOR_MAG)) {
            if (rcOptions[BOXMAG]) {
                if (!flightMode.MAG_MODE) {
                    flightMode.MAG_MODE = 1;
                    magHold = heading;
                }
            } else
                flightMode.MAG_MODE = 0;
            if (rcOptions[BOXHEADFREE]) {
                if (!flightMode.HEADFREE_MODE) {
                    flightMode.HEADFREE_MODE = 1;
                }
            } else {
                flightMode.HEADFREE_MODE = 0;
            }
        ...
        if (getSensors(SENSOR_GPS)) {
            if (flightMode.GPS_FIX && GPS_numSat >= 5) {
                if (rcOptions[BOXGPSHOME]) {
                    if (!flightMode.GPS_HOME_MODE) {
                        flightMode.GPS_HOME_MODE = 1;
                        GPS_set_next_wp(&GPS_home[LAT], &GPS_home[LON]);
                        nav_mode = NAV_MODE_WP;
                    }
                } else {
                    flightMode.GPS_HOME_MODE = 0;
                }
                if (rcOptions[BOXGPSHOLD]) {
                    if (!flightMode.GPS_HOLD_MODE) {
                        flightMode.GPS_HOLD_MODE = 1;
                    ...
                    flightMode.GPS_HOLD_MODE = 0;
                }
            }
        }

        if (rcOptions[BOXPASSTHRU]) {
            flightMode.PASSTHRU_MODE = 1;
        } else {
            flightMode.PASSTHRU_MODE = 0;
        }
    ...
    if (cfg.looptime == 0 || (int32_t)(currentTime - loopTime) >= 0) {
...
#ifdef MAG
        if (getSensors(SENSOR_MAG)) {
            if (abs(rcCommand[YAW]) < 70 && flightMode.MAG_MODE) {
        ...
        if (getSensors(SENSOR_BARO)) {
            if (flightMode.BARO_MODE) {
                if (abs(rcCommand[THROTTLE] - initialThrottleHold) > cfg.alt_hold_throttle_neutral) {
                    flightMode.BARO_MODE = 0;   // so that a new althold reference is defined
                }
        ...
        if (getSensors(SENSOR_GPS)) {
            ...
            if ((!flightMode.GPS_HOME_MODE && !flightMode.GPS_HOLD_MODE) || !flightMode.GPS_FIX_HOME) {
        ...
        for (axis = 0; axis < 3; axis++) {
            if ((flightMode.ANGLE_MODE || flightMode.HORIZON_MODE) && axis < 2) { // MODE relying on ACC
                // 70 degrees max inclination
                errorAngle = constrain(2 * rcCommand[axis] + GPS_angle[axis], -700, +700) - angle[axis] + cfg.angleTrim[axis];
#ifdef LEVEL_PDF
            ...
            if (!flightMode.ANGLE_MODE || axis == 2) { // MODE relying on GYRO or YAW axis
