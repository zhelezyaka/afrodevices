...
void buzzer(uint8_t warn_vbat)
{
    ...
    if (getFeature(FEATURE_FAILSAFE)) {
        if (failsafeCnt > (5 * cfg.failsafe_delay) && flightMode.ARMED) {
    ...
    if (getSensors(SENSOR_GPS)) {
        if ((rcOptions[BOXGPSHOME] || rcOptions[BOXGPSHOLD]) && !flightMode.GPS_FIX) {     // if no fix and gps funtion is activated: do warning beeps
            warn_noGPSfix = 1;
        } else {
            warn_noGPSfix = 0;
        }
    ...
    if (warn_failsafe == 2)
        ...
        beep_code('S','M','N','D');
    else if (warn_runtime == 1 && flightMode.ARMED)
