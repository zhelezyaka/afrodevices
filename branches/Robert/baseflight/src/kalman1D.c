...
void kalman1DUpdate32(kalman1D_t *kalmanState, int32_t *pvalue, float dt)
{
	...
	// Y = M – H*X
	float y0 = m - kalmanState->m_x[0];
	float y1 = ((m - oldX) / dt) - kalmanState->m_x[1];

	// S = H*P*H^T + R
	// Because H = [1, 0] this is easy, and s is a single value not a matrix to invert.
	float s = kalmanState->m_p[0] + kalmanState->m_r;

	// K = P * H^T *S^-1
	float k = kalmanState->m_p[0] / s;
