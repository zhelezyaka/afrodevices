...
static void airplaneMixer(void)
{
	...
	if (f.PASSTHRU_MODE)
	{
		...
		servo[3] = cfg.roll_direction_l * (rcData[ROLL] - mcfg.midrc);
		servo[4] = cfg.roll_direction_r * (rcData[ROLL] - mcfg.midrc);
		servo[5] = cfg.yaw_direction * (rcData[YAW] - mcfg.midrc);
		servo[6] = cfg.pitch_direction * (rcData[PITCH] - mcfg.midrc);
	}
	else
	{
		// use sensors to correct (gyro only or gyro + acc)
		servo[3] = cfg.roll_direction_l * (axisPID[ROLL] + (rcData[ROLL] - mcfg.midrc));
		servo[4] = cfg.roll_direction_r * (axisPID[ROLL] + (rcData[ROLL] - mcfg.midrc));
		servo[5] = cfg.yaw_direction * axisPID[YAW];
		servo[6] = cfg.pitch_direction * (axisPID[PITCH] + (rcData[PITCH] - mcfg.midrc));
	}
	...
	if (cfg.flaperons == 1)
	{ // Flaperon mix enabled
		servo[3] += rcData[cfg.flap_aux] - mcfg.midrc;
		servo[4] += rcData[cfg.flap_aux] - mcfg.midrc;
	}
...
void mixTable(void)
{
	...
	if (numberMotor > 1)
		for (i = 0; i < numberMotor; i++)
			motor[i] = rcCommand[THROTTLE] * currentMixer[i].throttle + axisPID[PITCH] * currentMixer[i].pitch + axisPID[ROLL] * currentMixer[i].roll
