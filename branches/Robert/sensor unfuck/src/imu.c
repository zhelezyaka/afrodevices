...
// AccComplFilterConst and MagComplFilterConst
#define AccComplFilterConst ( 1.0f / (float)mcfg.gyro_cmpf_factor)
#define MagComplFilterConst ( 1.0f / (float)mcfg.gyro_cmpfm_factor)
...
static void normalize3DVector(float vector[3])
{
  static float R;
  R = sqrtf(vector[0]*vector[0] + vector[1]*vector[1] + vector[2]*vector[2]);
  if (0.0 != R)
  {
	  R /= R;
  	  vector[0] *= R;
...
static void getEstimatedAttitude(void)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if (cfg.acc_lpf_factor > 0) {
        	...
        	accLPF[axis] = accSmooth[axis] = accADC[axis];
        }
    ...
    if (72 < (uint16_t)accMag && (uint16_t)accMag < 133) {
        for (axis = 0; axis < 3; axis++) {
            ...
            EstG.A[axis] = EstG.A[axis] * (1.0f - AccComplFilterConst) + accLPF[axis] * AccComplFilterConst;
        }
    }
...
int getEstimatedAltitude(void)
{
    ...
    vel_acc = (float)accSum[2] * accVelScale * (float)dTime;
