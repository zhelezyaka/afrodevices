...
static void evaluateCommand(void)
{
    ...
    case MSP_STATUS:
        ...
        serialize16(getSensors(SENSOR_ACC) | getSensors(SENSOR_BARO) << 1 | getSensors(SENSOR_MAG) << 2 | getSensors(SENSOR_GPS) << 3 | getSensors(SENSOR_SONAR) << 4);
        serialize32(flightMode.ANGLE_MODE << BOXANGLE | flightMode.HORIZON_MODE << BOXHORIZON | flightMode.BARO_MODE << BOXBARO | flightMode.MAG_MODE << BOXMAG | flightMode.ARMED << BOXARM | 
                    rcOptions[BOXCAMSTAB] << BOXCAMSTAB | rcOptions[BOXCAMTRIG] << BOXCAMTRIG | 
                    flightMode.GPS_HOME_MODE << BOXGPSHOME | flightMode.GPS_HOLD_MODE << BOXGPSHOLD | flightMode.HEADFREE_MODE << BOXHEADFREE | flightMode.PASSTHRU_MODE << BOXPASSTHRU | 
...
void serialCom(void)
{
    ...
    if (!cliMode && !uartAvailable() && getFeature(FEATURE_TELEMETRY) && flightMode.ARMED) { // The first 2 conditions should never evaluate to true but I'm putting it here anyway - silpstream
        sendTelemetry();
        return;
    }
}
