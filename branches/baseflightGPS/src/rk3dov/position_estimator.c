...
void read_accelerometer_raw_avg(int16_t accel_avg[3], int samples)
{
	extern sensor_t acc;
	...
	printf("[position_estimator] measuring...\n");
	while (count < samples)
	{
		// read in raw accel values
		acc.read(acc_raw);
		accel_sum[0] += acc_raw[0];
		accel_sum[1] += acc_raw[1];
		accel_sum[2] += acc_raw[2];
		count++;
	}
	for (int i = 0; i < 3; i++)
	{
	...
	printf("[position_estimator] raw data: [ %i  %i  %i ]\n", accel_avg[0], accel_avg[1], accel_avg[2]);
}
...
void do_accelerometer_calibration()
{
	...
	printf("[position_estimator] calibration started\n");

	printf("[position_estimator] place vehicle level, ");
	read_accelerometer_raw_avg(&(accel_raw_ref[5][0]),
	calibration_samples);
	printf("[position_estimator] place vehicle on it's back, ");
	read_accelerometer_raw_avg(&(accel_raw_ref[4][0]),
	calibration_samples);
	printf("[position_estimator] place vehicle on right side, ");
	read_accelerometer_raw_avg(&(accel_raw_ref[3][0]),
	calibration_samples);
	printf("[position_estimator] place vehicle on left side, ");
	read_accelerometer_raw_avg(&(accel_raw_ref[2][0]),
	calibration_samples);
	printf("[position_estimator] place vehicle nose down, ");
	read_accelerometer_raw_avg(&(accel_raw_ref[1][0]),
	calibration_samples);
	printf("[position_estimator] place vehicle nose up, ");
	read_accelerometer_raw_avg(&(accel_raw_ref[0][0]),
	calibration_samples);
	printf("[position_estimator] reference data collection done\n");

	int16_t accel_offs[3];
	float accel_T[3][3];
	int res = calculate_calibration_values(accel_raw_ref, accel_T, accel_offs, const_earth_gravity);
	if (res != 0)
	{
		printf(
		"[position_estimator] calibration parameters calculation error\n");
		return;
	}
	printf("[position_estimator] accelerometers raw offsets: [ %i  %i  %i ]\n", accel_offs[0], accel_offs[1], accel_offs[2]);
	printf("[position_estimator] accelerometers transform matrix:\n  [ %0.6f  %0.6f  %0.6f ]\n  | %0.6f  %0.6f  %0.6f |\n  [ %0.6f  %0.6f  %0.6f ]\n",
	...
	for (i = 0; i < 3; i++)
	{
		p.acc_offs[i] = accel_offs[i];
	}
	elems = sizeof(accel_T) / sizeof(accel_T[0][0]);
	for (i = 0; i < 3; i++)
	{
		for (n = 0; n < 3; i++)
		{
			p.acc_T[i][n] = accel_T[i][n];
		}
	}
...
//			{ /* poll only two first subscriptions */
...
//					static float local_pos_gps[3] =
//						{ 0.0f, 0.0f, 0.0f }; /* output variables from tangent plane mapping */
