...
void kalmanBaroUpdate(float measurement)
{
	...
	baro.p = baro.p + baro.q;

	//measurement update
	k = baro.p / (baro.p + baro.r);
	baro.x = baro.x + k * (measurement - baro.x);
	baro.p = (1 - k) * baro.p;
}
...
void baroKalmanfilterStep(int32_t *pressure)
{
	...
	{
		_init = 1;
#define Q 0.0008f			// process noise covariance
#define	R 1e-4f			// measurement noise covariance
