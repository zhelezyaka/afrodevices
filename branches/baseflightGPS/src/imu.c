...
#define GYRO_INTERLEAVE
    ...
    if (getFeature(FEATURE_GYRO_SMOOTHING)) {
...
#define MAG_VALUE magADC[axis]
    ...
    if (abs(accSmooth[ROLL]) < acc_25deg && abs(accSmooth[PITCH]) < acc_25deg && accSmooth[YAW] > 0)
        ...
        flightMode.SMALL_ANGLES_25 = 0;

    // Apply complimentary filter (Gyro drift correction)
    // If accel magnitude >1.4G or <0.6G and ACC vector outside of the limit range => we neutralize the effect of accelerometers in the angle estimation.
    // To do that, we just skip filter, as EstV already rotated by Gyro
    if ((36 < accMag && accMag < 196) || flightMode.SMALL_ANGLES_25) {
        for (axis = 0; axis < 3; axis++)
...
//    accel_ned.V.Z = accSmooth[2];
	accel_ned.V.X = applyDeadband16(accSmooth[0], cfg.accz_deadband);
	accel_ned.V.Y = applyDeadband16(accSmooth[1], cfg.accz_deadband);
	accel_ned.V.Z = applyDeadband16(accSmooth[2], cfg.accz_deadband);
